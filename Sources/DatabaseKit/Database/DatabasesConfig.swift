...
public struct DatabasesConfig: Service {
    ...
    public mutating func add<D>(as id: DatabaseIdentifier<D>, database: @escaping LazyDatabase<D>) {
        databases[id.uid] = database
    }

    /// Enables logging on the supplied database
    public mutating func enableLogging<D>(on database: DatabaseIdentifier<D>, logger: DatabaseLogger = .print) where D: LogSupporting {
